class Comments {
    ...
    fetchComments() {
        let url = `${this.phpFileUrl}?article_id=${this.pageId}`;
    ...
    _eventHandlers() {
        this.container.querySelectorAll(".comments .write_comment_btn, .comments .reply_comment_btn").forEach(element => {
        ...
        this.container.querySelectorAll(".comments .write_comment form").forEach(element => {
            element.onsubmit = event => {
                event.preventDefault();
                fetch(`${this.phpFileUrl}?article_id=${this.pageId}`, {
                    method: 'POST',
        ...
        this.container.querySelectorAll(".comments .vote").forEach(element => {
            element.onclick = event => {
                event.preventDefault();
                fetch(`${this.phpFileUrl}?article_id=${this.pageId}&vote=${element.getAttribute("data-vote")}&comment_id=${element.getAttribute("data-comment-id")}`).then(response => response.text()).then(data => {
